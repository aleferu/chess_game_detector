...
from common import setup_logging, PIECE_MAP
import torchinfo


class MicroPieceCNN(nn.Module):
    """
    Lightweight CNN for Piece Type Classification (12 classes).
    Input: [B, 1, 64, 64] -> Output: [B, 12]
    """
...
class PieceTypeDataset(Dataset):
    def __init__(self, root_dir, csv_file, img_size=64, train=True, augment=True):
        ...
        # Map string labels to 0-11 indices (skipping empty)
        self.class_map = {k: v-1 for k, v in PIECE_MAP.items() if k != ""}
        # Inverse map for logging
        self.idx_to_class = {v: k for k, v in self.class_map.items()}

        # Track counts for weighting
        self.class_counts = {i: 0 for i in range(12)}
    ...
    def get_class_weights(self, device):
        ...
        for i in range(num_classes):
            ...
            # Avoid division by zero if a class is missing (unlikely but safe)
            w = total_samples / (num_classes * max(count, 1))
            weights.append(w)
            logging.info(f"  Class {i} ({self.idx_to_class[i]}): {count} samples -> Weight: {w:.4f}")
...
@torch.no_grad()
def evaluate(model, loader, device, criterion, num_classes=12):
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total = 0
...
def train(args):
    setup_logging()
    logging.info(f"Starting Piece Classification training with config: {vars(args)}")
    ...
    # Data
    full_ds = PieceTypeDataset(args.data_dir, args.csv_file, img_size=args.img_size, train=True, augment=True)
    idx_to_class = full_ds.idx_to_class

    # Calculate Weights based on full dataset (approximation for train split)
    ...
    val_base_ds = PieceTypeDataset(args.data_dir, args.csv_file, img_size=args.img_size, train=False, augment=False)
    ...
    # Model
    model = MicroPieceCNN(num_classes=12).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Validation
        val_metrics = evaluate(model, val_loader, device, criterion, num_classes=12)
        ...
        # Header
        header = "      " + " ".join([f"{idx_to_class.get(i, str(i)):>4}" for i in range(12)])
        logging.info(header)
        for i in range(12):
            row_str = f"{idx_to_class.get(i, str(i)):>4}: " + " ".join([f"{cm[i, j]:>4}" for j in range(12)])
        ...
        if val_metrics["loss"] < best_val_loss:
            ...
            epochs_no_improve = 0
            torch.save(model.state_dict(), os.path.join(args.out_dir, "best_piece_model.pth"))
        ...
        # Save last
        torch.save(model.state_dict(), os.path.join(args.out_dir, "last_piece_model.pth"))
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="out/piece_model")
